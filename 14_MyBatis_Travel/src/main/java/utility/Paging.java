...
public class Paging {
	...
	public Paging(
		...
		this.limit = pageSize ; // 한번에 몇페이지까지 보이게할지

		this.totalCount = totalCount ; 

		this.totalPage = (int)Math.ceil((double)this.totalCount / this.pageSize) ; // 전체필요한 페이지 개수 , 9.0/2 = 4.5 => 5.0

		this.beginRow = ( this.pageNumber - 1 )  * this.pageSize  + 1 ; // (2-1) * 2+1 = 3 , 몇번부터 몇번까지 나올지 정함
		...
		this.beginPage = ( this.pageNumber - 1 ) / this.pageCount * this.pageCount + 1  ; // (2-1) / 3 * 3 + 1 = 
		this.endPage = this.beginPage + this.pageCount - 1 ; // 2 + 3 - 1 = 4

		if( this.pageNumber > this.totalPage ){  
			this.pageNumber = this.totalPage ;
		}
		
		this.offset = ( pageNumber - 1 ) * pageSize ; // 건너띌 레코드 갯수
		...
		<a href='/ex/list.tv?pageNumber=5&pageSize=2&whatColumn=" + whatColumn + "&keyword=" + keyword'>5
		<a href='/ex/list.tv?pageNumber=6&pageSize=2&whatColumn=" + whatColumn + "&keyword=" + keyword'>6
